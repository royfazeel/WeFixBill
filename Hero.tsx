...
// Animated counter with motion-safe check
...
export default function Hero({ onOpenModal }: HeroProps) {
  const stats = [
    { value: 2500000, prefix: '$', label: 'Total Saved', color: 'text-stripe-purple' },
    { value: 15000, suffix: '+', label: 'Happy Customers', color: 'text-stripe-blue' },
    { value: 127, prefix: '$', suffix: '/mo', label: 'Avg. Savings', color: 'text-stripe-cyan' },
    { value: 94, suffix: '%', label: 'Success Rate', color: 'text-stripe-green' },
  ]
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div 
          className="absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-stripe-purple/10 rounded-full blur-[150px]"
          animate={{ 
            scale: [1, 1.1, 1],
            opacity: [0.3, 0.5, 0.3],
          }}
        ...
        />
        <motion.div 
          className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-stripe-cyan/10 rounded-full blur-[120px]"
          animate={{ 
            scale: [1, 1.15, 1],
            opacity: [0.2, 0.4, 0.2],
          }}
      ...
      >
        <motion.div variants={fadeInUp} className="max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-stripe-purple" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="text-sm font-medium">4.9/5 Rating</span>
            </div>
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-stripe-blue" fill="currentColor" viewBox="0 0 20 20">
      ...
      >
        ...
        >
          <motion.div
            ...
            transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
            className="w-1.5 h-2.5 bg-stripe-purple rounded-full"
          />
        </motion.div>
      </motion.div>
    </section>
